...
- **Sprites are not palette-constrained**: The current sprites were generated by Retro Diffusion and have not been run through the Aseprite cleanup workflow. Accent pixels likely don't exactly match the 4 source ramp hex values, so the pixel-level remap silently does nothing on most sprites — every song actually still shows the same default pink accent. The ramp swap code is correct; the sprites are the blocker.
...
- **The palette ramp swap is done in code, but invisible until sprites are fixed.** This is an unusual state: the feature is implemented and tested, but its visual effect cannot be seen because of a sprite asset issue. The Aseprite workflow in SPRITES.md is clear and complete. Running it would close a two-cycle visual gap with no code changes at all.
...
1. **Run the Aseprite palette-constraint workflow on the sprites** — The ramp swap has been implemented for two cycles but is invisible because sprites aren't palette-constrained; this one asset-cleanup task makes cycle 006's work actually visible. No code changes needed, just run the Aseprite → `check-palette.py` → `slice-sprites.py` pipeline documented in SPRITES.md.

2. **Add sorting/ordering to the song list** — A sort-by-updated_at (most recently edited first) would make the app meaningfully more useful as a sketchbook immediately; it requires a trivial SQL `ORDER BY` change plus a small tRPC/client update with no schema migration.
