...
export const appRouter = t.router({
  song: t.router({
    ...
    create: t.procedure.input(CreateSongInput).mutation(({ input }) => {
      ...
      const title = input.title || generateTitle()

      const stmt = db.prepare<
        [string, string, string, null | string, string, string, string, string, string]
      >(
        `
    ...
    // Delete a song
    delete: t.procedure.input(z.object({ id: z.string().uuid() })).mutation(({ input: { id } }) => {
      const db = getDb()
